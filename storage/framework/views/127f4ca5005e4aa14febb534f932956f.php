...
<div x-data="{ open: false, action: '', title: '' }" class="space-y-12 animate-in fade-in slide-in-from-bottom duration-700">
    <!-- Header Section -->
                    ...
                    <tr class="hover:bg-slate-50/30 transition-all group">
                        <td class="px-10 py-6">
                            <div class="flex flex-col">
                                <span class="font-extrabold text-slate-900 text-lg leading-tight group-hover:text-cyan-700 transition-colors"><?php echo e($loan->book->title); ?></span>
                                <span class="text-[10px] font-bold text-slate-300 uppercase tracking-widest mt-1">ISBN: <?php echo e($loan->book->isbn); ?></span>
                            </div>
                        </td>
                        <td class="px-10 py-6">
                            <div class="flex flex-col">
                                <span class="font-bold text-slate-800"><?php echo e($loan->member->name); ?></span>
                                <span class="text-[10px] font-bold text-slate-400"><?php echo e($loan->member->email); ?></span>
                            </div>
                        </td>
                        <td class="px-10 py-6">
                            <div class="flex flex-col">
                                <span class="text-slate-600 font-bold"><?php echo e(\Carbon\Carbon::parse($loan->loan_date)->translatedFormat('d M Y')); ?></span>
                                <?php if($loan->return_date): ?>
                                    <span class="text-[9px] text-emerald-500 font-black uppercase tracking-widest mt-1 bg-emerald-50 px-2 py-0.5 rounded-full self-start">Kembali: <?php echo e(\Carbon\Carbon::parse($loan->return_date)->translatedFormat('d M Y')); ?></span>
                                <?php endif; ?>
                            </div>
                        </td>
                        <td class="px-10 py-6">
                        ...
                        <td class="px-10 py-6">
                            <div class="flex items-center justify-end gap-3 transition-all duration-300">
                                <?php if($loan->status === 'borrowed'): ?>
                                    <form action="/loans/<?php echo e($loan->id); ?>/return" method="POST">
                                ...
                                <?php endif; ?>
                                <button @click="open = true; action = '/loans/<?php echo e($loan->id); ?>'; title = '<?php echo e($loan->book->title); ?> oleh <?php echo e($loan->member->name); ?>'" 
    ...
    <div x-show="open" 
         ...
         class="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-950/40 backdrop-blur-md" 
         style="display: none;">
        ...
        <div @click.away="open = false" 
            ...
            <div class="space-y-3">
                <h2 class="text-3xl font-black text-slate-900 tracking-tight">Hapus Riwayat?</h2>
                <p class="text-slate-400 text-sm font-medium leading-relaxed">Hapus catatan transaksi <span class="text-slate-900 font-bold" x-text="title"></span>? Tindakan ini tidak dapat dibatalkan.</p>
            </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/infrastruktur/Documents/RPL/Project/sample/resources/views/loans/index.blade.php ENDPATH**/ ?>