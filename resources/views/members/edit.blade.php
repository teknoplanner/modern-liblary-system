...
<div class="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom duration-500">
    ...
    <form action="/members/{{ $member->id }}" method="POST" class="bg-white p-8 rounded-3xl shadow-xl shadow-gray-100 border border-gray-100 space-y-6">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 gap-6">
            ...
            <div class="space-y-2">
                ...
                <textarea name="address" id="address" rows="3"
                    class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all resize-none">{{ $member->address }}</textarea>
            </div>
        </div>
